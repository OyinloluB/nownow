...
//Success actions
export const updateDistributorPasswordSuccess = () => ({
  type: PasswordAction.PASSWORD_DISTRIBUTOR_SUCCESS,
});

export const updateBulkBreakerPasswordSuccess = () => ({
  type: PasswordAction.PASSWORD_BULK_BREAKER_SUCCESS,
});

export const updatePocPasswordSuccess = () => ({
...
export const updateDistributorPassword = (ID, password) => {
  return (dispatch) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const response = await axios.patch("/Distributor/Password", {
          ID,
          password,
        });
        const { data } = response;
...
export const updateBulkBreakerPassword = (ID, password) => {
  return (dispatch) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const response = await axios.patch("/BulkBreaker/Password", {
          ID,
          password,
        });
        const { data } = response;
